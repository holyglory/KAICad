...
public sealed class InstanceTools(InstanceRegistry registry)
{
    ...
    public Task<CallToolResult> Capabilities(string instanceId, CancellationToken cancellationToken) => InstanceToolBoundary.Run(async () =>
    {
        ...
        var service = new[]
        {
            ...
            ,new InstanceCapability("kicad_pcb_route_candidate_commit", "pcb-routing", "compiled-mcp plus native-board-api", "registered", "complete detached DRC qualification, board lifecycle state, atomic undoable commit", false)
