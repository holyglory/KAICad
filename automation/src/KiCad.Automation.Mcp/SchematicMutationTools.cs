...
public sealed class SchematicMutationTools(InstanceRegistry registry)
{
    [McpServerTool(Name = "kicad_schematic_move_connected_symbols", UseStructuredContent = true,
     ...
     Description("Move explicitly identified symbols using native connected drag, retaining attached wiring and native undo. Requires the exact loaded sheet document JSON, symbol UUIDs, displacement in nm (multiples of 100), document epoch, observed revision sequence and a caller-stable operation ID. A non-displayed physical sheet is edited without navigating or replacing the user's view and selection. Repeated instances sharing the visible screen, or requiring different loaded symbol units, remain unsupported. Reuse identical arguments and operation ID after a timeout; never retry with a fresh ID without inspecting the old receipt. Returns changed/deleted native items, not a matched image or a complete electrical correctness certificate. Complete revision tracking remains unavailable. Cancellation of the MCP request does not prove native rollback.")]
