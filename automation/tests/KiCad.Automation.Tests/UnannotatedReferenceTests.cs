...
public sealed class UnannotatedReferenceTests
{
    ...
    public void DuplicateNativePlaceholdersKeepDistinctComponentAndPinIdentities(string reference)
    {
        ...
        var circuit = original with { Components = original.Components.Select(c => c with { Reference = reference }).ToArray() };
        string xml = CircuitXml.Write(circuit); var read = CircuitXml.Read(xml);
        Assert.AreEqual(xml, CircuitXml.Write(read));
        Assert.HasCount(2, read.Components); Assert.AreNotEqual(read.Components[0].Id, read.Components[1].Id);
    ...
    {
        var original = CircuitXmlTests.Fixture(); string before = CircuitXml.Write(original);
        var duplicate = original with { Components = original.Components.Select(c => c with { Reference = reference }).ToArray() };
    ...
    {
        var state = SchematicSynchronizationPlanTests.Fixture(); var original = state.Baseline;
        var circuit = original.Engineering.Circuit with { Components = original.Engineering.Circuit.Components
            .Select(c => c with { Reference = "U?" }).ToArray() };
        var unannotated = SchematicPropertyProjection.Project(original, original with
            { Engineering = original.Engineering with { Circuit = circuit } }, state.KnowledgeLibraries);
        var initial = SchematicDesignBindings.Inspect(unannotated, state.KnowledgeLibraries);
        Assert.IsTrue(initial.IdentitiesResolved); Assert.IsEmpty(initial.Differences);
        ...
        var electrical = state.ObservedElectrical!.Clone(); electrical.Hierarchy.Data = reconstructed.Schematic.Clone();
        ...
        var assigned = reconstructed.Engineering with { Circuit = reconstructed.Engineering.Circuit with
            { Components = reconstructed.Engineering.Circuit.Components.Select((c, i) => c with { Reference = "U" + (i + 100) }).ToArray() } };
        var annotated = SchematicPropertyProjection.Project(reconstructed, reconstructed with { Engineering = assigned }, state.KnowledgeLibraries);
        var reverse = SchematicModelProjection.Reconcile(reconstructed, reconstructed.Engineering, annotated.Schematic, state.KnowledgeLibraries);
        ...
        var undone = SchematicModelProjection.Reconcile(annotated, annotated.Engineering, reconstructed.Schematic, state.KnowledgeLibraries);
        ...
        CollectionAssert.AreEquivalent(original.Engineering.Circuit.Nets.Single().Pins.ToArray(), undone.Candidate.Circuit.Nets.Single().Pins.ToArray());
